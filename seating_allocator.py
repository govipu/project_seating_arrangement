"""
seating_allocator.py — Core logic for exam seating arrangement generation (final).
"""
...
# from config import ROOMS, ADJACENCY
    ...
    def load_inputs(self):
        ...
        try:
            ...
            if 'in_roll_name_mapping' in self.sheets:
                ...
                # Accept columns 'Roll' and 'Name' (case-sensitive as provided); be tolerant:
                # try variations if not present
                if 'Roll' in df_names.columns and 'Name' in df_names.columns:
                    for _, r in df_names.iterrows():
                        roll = str(r['Roll']).strip()
                        name = str(r['Name']).strip() or 'Unknown Name'
                        if roll:
                            self.roll_name_map[roll] = name
                else:
                    # attempt lowercase variants
                    cols_lower = {c.lower(): c for c in df_names.columns}
                    if 'roll' in cols_lower and 'name' in cols_lower:
                        for _, r in df_names.iterrows():
                            roll = str(r[cols_lower['roll']]).strip()
                            name = str(r[cols_lower['name']]).strip() or 'Unknown Name'
                            if roll:
                                self.roll_name_map[roll] = name
                    else:
                        self.logger.warning("in_roll_name_mapping does not have expected 'Roll'/'Name' columns; names will default to 'Unknown Name'")
                self.logger.info("Loaded %d roll-name entries.", len(self.roll_name_map))
            else:
                self.logger.warning("in_roll_name_mapping sheet not present; all names will be 'Unknown Name'.")
